from datafeeder_v2_rgb_weighted_challenge1 import DataFeeder
#from datafeedCPP import DataFeederSAMP
from fusionresnet_challenge1_alldata import ResNetFusionModel
...
def datafeeder(path, train_split,batch_size, image_size, skipdata='missing.csv'):
    csv_data = pd.read_csv(os.path.join(path,'terraclimate_pseudoabsence_lc_updatedname.csv'))
...
def train(hyperparameters, resume_training=False):
        ...
        weights_dir = "weights_challenge1_fusion_lc_nume_bce_updated_sigmoid"
        ...
        log_file_path = "metrics/metrics_challenge1_fusion_lc_nume_bce_updated_sigmoid.csv"
        if os.path.isfile(log_file_path):
            ...
            writer = csv.writer(log_file)
            writer.writerow(["Epoch", "Train Loss", "Train MSE", "Train MAE", "Train Accuracy",  "Test Loss", "Test MSE", "Test MAE"," Test Accuracy", "learning rate", "Batch size", "fpr", "tpr", "thresholds", "auc_score", "tpr_tr", "thresholds_tr", "auc_score_tr"])
        ...
        log_dir = "logs_challenge1_fusion_lc_nume_bce_updated_sigmoid"
        ...
        for epoch in range(start_epoch, epochs + 1):
            ...
            #for [image_batch, numeric_batch], labels_batch, weights in tqdm(train_feeder, desc='Train'):
            #for [image_batch, labels_batch], weights in tqdm(train_feeder, desc='Train'):
                batch_num+=1
                ...
                #train_probabilities =logits.numpy()
                #fpr_tr, tpr_tr, thresholds_tr = roc_curve(labels_batch, train_probabilities)
                #auc_score_tr = roc_auc_score(labels_batch, train_probabilities)
            ...
            #for [image_batch, numeric_batch], labels_batch, weights in tqdm(test_feeder, desc='Test'):
                ...
                test_probabilities =test_logits.numpy()
                fpr, tpr, thresholds = roc_curve(labels_batch, test_probabilities)
                auc_score = roc_auc_score(labels_batch, test_probabilities)
            ...
            if epoch % hyperparameters["save_weight_epoch"] == 0:
                ...
                model_filename = f"resnet_fusion_model_lc_nume_bce_sigmoid_ch1_epoch{epoch + 1}_acc{test_accuracy.result().numpy():.4f}.h5"
                model.save_weights(os.path.join(weights_dir, model_filename))

            writer.writerow([epoch + 1, train_loss.result().numpy(),  train_mse.result().numpy(), train_mae.result().numpy(), train_accuracy.result().numpy(),test_loss.result().numpy(),  test_mse.result().numpy(), test_mae.result().numpy(), test_accuracy.result().numpy(), hyperparameters["LR"], hyperparameters["batch_size"], fpr, tpr, thresholds, auc_score])
            ...
            print(f"Epoch: {epoch}, Test  Loss: {test_loss.result()},  Test MSE:    {test_mse.result()}, Test MAE: {test_mae.result()}, Test Accuracy: {test_accuracy.result()}, AUC : {auc_score}")
...
if __name__ == '__main__':
    ...
    countries = ["australia", "costarica_new2", "southafrica","pseudo_absence"]
    csv_file = "Sentinel_2/terraclimate_pseudoabsence_lc_updatedname.csv"
    train_split = 0.8
    is_train = True
